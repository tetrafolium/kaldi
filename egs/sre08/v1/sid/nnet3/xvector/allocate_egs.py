...
def main():
    ...
    info_f = open(args.egs_dir + "/temp/" + prefix +
                  "archive_chunk_lengths", "w")
    if info_f is None:
        sys.exit(str("Error opening file {0}/temp/" + prefix +
    ...
    for archive_index in range(args.num_archives):
        ...
        if args.randomize_chunk_length == "true":
            ...
            length = deterministic_chunk_length(
                archive_index, args.num_archives, args.min_frames_per_chunk, args.max_frames_per_chunk)
    ...
    for i in range(0, args.num_archives):
        num_archives_per_job[i %
                             args.num_jobs] = num_archives_per_job[i % args.num_jobs] + 1
    ...
    for job in range(args.num_jobs):
        ...
        f = open(args.egs_dir + "/temp/" + prefix +
                 "ranges." + str(job + 1), "w")
        if f is None:
            sys.exit("Error opening file " + args.egs_dir +
                     "/temp/" + prefix + "ranges." + str(job + 1))
    ...
    print("allocate_egs.py: finished generating " + prefix +
          "ranges.* and " + prefix + "outputs.* files")
