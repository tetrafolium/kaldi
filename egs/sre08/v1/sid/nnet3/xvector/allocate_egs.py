...
def get_args():
    ...
    parser.add_argument(
        ...
        default="",
        help=
        "Adds a prefix to the output files. This is used to distinguish between the train "
    ...
    parser.add_argument(
        ...
        default=-1,
        help=
        "Number of jobs we're going to use to write the archives; the ranges.* "
...
def deterministic_chunk_length(archive_id, num_archives, min_frames_per_chunk,
...
def main():
    ...
    info_f = open(args.egs_dir + "/temp/" + prefix + "archive_chunk_lengths",
                  "w")
    if info_f is None:
        sys.exit(
            str("Error opening file {0}/temp/" + prefix +
                "archive_chunk_lengths").format(args.egs_dir))
    for archive_index in range(args.num_archives):
        ...
        if args.randomize_chunk_length == "true":
            ...
            length = random_chunk_length(args.min_frames_per_chunk,
                                         args.max_frames_per_chunk)
        else:
            length = deterministic_chunk_length(archive_index,
                                                args.num_archives,
        ...
        for n in range(this_num_egs):
            if len(spkrs) == 0:
                print("Ran out of speakers for archive {0}".format(
                    archive_index + 1))
                break
            spkr = spkrs.pop()
    ...
    for i in range(0, args.num_archives):
        num_archives_per_job[
            i % args.num_jobs] = num_archives_per_job[i % args.num_jobs] + 1
    ...
    for job in range(args.num_jobs):
        ...
        f = open(args.egs_dir + "/temp/" + prefix + "ranges." + str(job + 1),
                 "w")
        if f is None:
            sys.exit("Error opening file " + args.egs_dir + "/temp/" + prefix +
                     "ranges." + str(job + 1))
        for (utterance_index, i, offset) in sorted(this_ranges):
            archive_index = this_archives_for_job[i]
            print("{0} {1} {2} {3} {4} {5}".format(
                utterance_index, i, archive_index + 1, offset,
        ...
        f = open(args.egs_dir + "/temp/" + prefix + "outputs." + str(job + 1),
                 "w")
        if f is None:
            sys.exit("Error opening file " + args.egs_dir + "/temp/" + prefix +
                     "outputs." + str(job + 1))
        print(" ".join([
            str("{0}/" + prefix + "egs_temp.{1}.ark").format(
                args.egs_dir, n + 1) for n in this_archives_for_job
        ]),
              file=f)
        f.close()
    ...
    print("allocate_egs.py: finished generating " + prefix + "ranges.* and " +
          prefix + "outputs.* files")
