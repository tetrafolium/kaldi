...
def VB_diarization(X,
    ...
    alphaQInit  - Dirichlet concentration parameter for initializing q
    ...
    mixture_sum = coo_matrix((np.ones(C * D),
                              (np.repeat(range(C), D), range(C * D))),
                             shape=(C, C * D))
    ...
    for ii in range(maxIters):
        ...
        for sid in range(maxSpeakers):
            # eq. (34) inverse
            invL = np.linalg.inv(np.eye(R) + tril_to_sym(VtNsiEV_flat[sid]))
            # eq. (35)
        ...
        sp = q[0, ::minDur] + np.exp(
            ...
            lb[1:, ::minDur] + lls[1:] + np.log(
                (1 - loopProb) * sp) - tll).sum(0)
        sp = sp / sp.sum()
...
def DER(q, ref, expected=True, xentropy=False):
    ...
    acc = [
        err_mx[perm[:err_mx.
                    shape[1]], range(err_mx.shape[1])].sum()
