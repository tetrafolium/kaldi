...
def write_segment(start_time, end_time, text, reco_id, segments_fh, utt2spk_fh,
                  text_fh):
    assert end_time > start_time

    text = normalize_text(text)

    utt_id = "{reco_id}-{st:06d}-{end:06d}".format(reco_id=reco_id,
...
def parse_calls_transcript_file(transcript_file, segments_fh, utt2spk_fh,
                                text_fh):
    ...
    for line in open(transcript_file):
        ...
        if len(parts) == 1:
            ...
            write_segment(inline_start_time, start_time, inline_text,
                          file_id + "_inLine", segments_fh, utt2spk_fh,
                          text_fh)
            write_segment(outline_start_time, start_time, outline_text,
                          file_id + "_outLine", segments_fh, utt2spk_fh,
                          text_fh)
            break
        ...
        if parts[1] == "inLine":
            ...
            if inline_start_time >= 0:
                write_segment(inline_start_time, start_time, inline_text,
                              reco_id, segments_fh, utt2spk_fh, text_fh)
...
def parse_non_calls_transcript_file(transcript_file, segments_fh, utt2spk_fh,
                                    text_fh):
    base_name = os.path.basename(transcript_file)
    file_id = re.sub(".transcription.txt", "", base_name)
    ...
    with open(transcript_file) as fh:
        ...
        while text != '':
            ...
            write_segment(start_time, end_time, text, file_id, segments_fh,
                          utt2spk_fh, text_fh)
            start_time = end_time
            text = fh.readline()
...
if __name__ == "__main__":
    ...
    for line in calls_list:
        ...
        wav_file = "{root_path}/src/{file_id}.wav".format(root_path=root_path,
                                                          file_id=file_id)

        for channel in [1, 2]:
            reco_id = file_id + ("_inLine" if channel == 1 else "_outLine")
            print("{reco_id} {file_id} {channel}"
                  "".format(reco_id=reco_id,
                            file_id=file_id,
        ...
        parse_calls_transcript_file(transcript_file, segments_fh, utt2spk_fh,
                                    text_fh)
