...
parser.add_argument(
    "num_cj5_states", type=int, help="number of states for punctuation")
parser.add_argument(
    "nonsilence_phones",
    type=str,
    help=
...
for x in range(0, args.num_nonsil_states):
    xp1 = x + 1
    print("<State> " + str(x) + " <PdfClass> " + str(x) + " <Transition> " +
          str(x) + " 0.75 <Transition> " + str(xp1) + " 0.25 </State>")
...
if (args.num_sil_states > 1):
    ...
    for x in range(0, (args.num_sil_states - 1)):
        state_str = state_str + "<Transition> " + \
            str(x) + " " + str(transp) + " "
