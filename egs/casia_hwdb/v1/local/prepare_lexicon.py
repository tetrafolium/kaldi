...
with open(os.path.join(args.dir, 'cj5-cc.txt'), 'r', encoding='utf-8') as f:
    for line in f:
        ...
        if not line_vect[0].startswith('yyy') and not line_vect[0].startswith(
                'z'):
...
with open(text_path, 'r', encoding='utf-8') as f:
    for line in f:
        ...
        for i in range(1, len(line_vect)):
            ...
            characters = " ".join([
                'SIL' if char == '|' else char if char in radical else
                cj5_table[char] if char in cj5_table else char
