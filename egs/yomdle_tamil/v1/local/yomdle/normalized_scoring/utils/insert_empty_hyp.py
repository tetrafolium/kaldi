...
if len(sys.argv) != 4:
    print(
        "Usage: insert_empty_hyp.py <ids-to-insert> <in-hyp-file> <out-hyp-file>"
    )
    sys.exit(1)
...
def main():
    ...
    with open(
            hyp_in_file, 'r', encoding='utf-8') as hyp_in_fh, open(
                hyp_out_file, 'w', encoding='utf-8') as hyp_out_fh, open(
                    ids_file, 'r') as ids_fh:
