...
parser.add_argument(
    'dir', help='directory of images.scp and is also output directory')
...
def get_scaled_image(im):
    ...
    im_pad = np.concatenate(
        (255 * np.ones((padding_y, math.ceil(1.0 * padding_x / 2)), dtype=int), im), axis=1)
    im_pad1 = np.concatenate(
        (im_pad, 255 * np.ones((padding_y, int(1.0 * padding_x / 2)), dtype=int)), axis=1)
    return im_pad1
