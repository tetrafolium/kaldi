...
def parse_splice_string(splice_string):
    ...
    try:
        ...
        raise Exception('Unknown format in splice_indexes variable: {0}'.format(
...
def create_config_files(output_dir, params):
    ...
    try:
        nnet_config = ["SpliceComponent input-dim={0} context={1} const-component-dim={2}".format(params.total_input_dim, contexts[0], params.ivector_dim),
                       "FixedAffineComponent matrix={0}".format(
                           params.lda_mat),
                       "AffineComponentPreconditionedOnline input-dim={0} output-dim={1} {2} learning-rate={3} param-stddev={4} bias-stddev={5}".format(
                           params.lda_dim, pnorm_input_dim, params.online_preconditioning_opts, params.initial_learning_rate, stddev, params.bias_stddev),
                       ("PnormComponent input-dim={0} output-dim={1} p={2}".format(pnorm_input_dim, pnorm_output_dim, pnorm_p)
                        if pnorm_input_dim != pnorm_output_dim else "RectifiedLinearComponent dim={0}".format(pnorm_input_dim)),
...
if __name__ == "__main__":
    ...
    parser.add_argument('--total-input-dim', type=int,
    ...
    parser.add_argument('--lda-mat', type=str,
                        help='lda-matrix used after the first splice component')
    parser.add_argument('--lda-dim', type=str,
    ...
    if params.mode == "contexts":
        [context, context_variables] = parse_splice_string(
            params.splice_indexes)
