...
def ProcessUtterance(split_lines_of_utt):
    ...
    if args.allow_repetitions == 'true':
        new_split_lines_of_utt = ProcessUtteranceForRepetitions(
            new_split_lines_of_utt)
...
def ProcessData():
    ...
    while True:
        ...
        if len(split_pending_line) == 0:
            if next_line != '':
                sys.exit(
                    "modify_ctm_edits.py: got an empty or whitespace input line"
                )
    try:
        f_out.close()
    except:
...
def PrintNonScoredStats():
    ...
    percent_lines_incorrect = '%.2f' % (num_incorrect_lines * 100.0 /
                                        num_lines)
    ...
    keys = sorted(ref_change_stats.keys(),
                  reverse=True,
...
def PrintRepetitionStats():
    ...
    keys = sorted(repetition_stats.keys(),
                  reverse=True,
