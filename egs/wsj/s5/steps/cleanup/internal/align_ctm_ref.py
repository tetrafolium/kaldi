...
def get_args():
    ...
    parser.add_argument("--hyp-format",
                        type=str,
                        choices=["Text", "CTM"],
                        default="CTM",
    ...
    parser.add_argument("--reco2file-and-channel",
                        ...
                        required.""")
    parser.add_argument("--eps-symbol",
                        type=str,
                        default="-",
    ...
    parser.add_argument("--correct-score",
                        type=int,
                        default=1,
                        help="Score for correct matches")
    parser.add_argument("--substitution-penalty",
                        type=int,
                        default=1,
                        help="Penalty for substitution errors")
    parser.add_argument("--deletion-penalty",
                        type=int,
                        default=1,
                        help="Penalty for deletion errors")
    parser.add_argument("--insertion-penalty",
                        type=int,
                        default=1,
                        help="Penalty for insertion errors")

    parser.add_argument("--align-full-hyp",
                        type=str,
...
def read_text(text_file):
    ...
    for line in text_file:
        ...
        if len(parts) < 1:
            raise RuntimeError("Did not get enough columns; line {0} in {1}"
                               "".format(line, text_file.name))
        elif len(parts) == 1:
            logger.warn("Empty transcript for utterance %s in %s", parts[0],
                        text_file.name)
            yield parts[0], []
        else:
...
def smith_waterman_alignment(ref,
                             hyp,
        ...
        if align_full_hyp and ref_index == 0:
            for hyp_index in range(1, hyp_len + 1):
                ...
                logger.debug("({0},{1}) -> ({2},{3}): {4}"
                             "".format(ref_index, hyp_index - 1, ref_index,
                                       hyp_index, H[ref_index][hyp_index]))
        ...
        for hyp_index in range(1, hyp_len + 1):  # Hypothesis
            ...
            if ((not align_full_hyp and sub_or_ok > 0) or
                ...
                logger.debug("({0},{1}) -> ({2},{3}): {4} ({5},{6})"
                             "".format(ref_index - 1, hyp_index - 1, ref_index,
                                       hyp_index, H[ref_index][hyp_index],
                                       ref[ref_index - 1], hyp[hyp_index - 1]))

            if H[ref_index -
                 1][hyp_index] + del_score > H[ref_index][hyp_index]:
                H[ref_index][hyp_index] = H[ref_index -
                                            1][hyp_index] + del_score
                bp[ref_index][hyp_index] = (ref_index - 1, hyp_index)
                logger.debug("({0},{1}) -> ({2},{3}): {4}"
                             "".format(ref_index - 1, hyp_index, ref_index,
                                       hyp_index, H[ref_index][hyp_index]))

            if H[ref_index][hyp_index -
                            1] + ins_score > H[ref_index][hyp_index]:
                H[ref_index][hyp_index] = H[ref_index][hyp_index -
                                                       1] + ins_score
                bp[ref_index][hyp_index] = (ref_index, hyp_index - 1)
                logger.debug("({0},{1}) -> ({2},{3}): {4}"
                             "".format(ref_index, hyp_index - 1, ref_index,
    ...
    score = max_score
    logger.debug("Alignment score: %s for (%d, %d)", score, ref_index,
                 hyp_index)
    ...
    while ((not align_full_hyp and score >= 0)
        ...
        try:
            ...
            if (ref_index == prev_ref_index + 1
                ...
                output.append(
                    (ref[ref_index - 1] if ref_index > 0 else eps_symbol,
                     eps_symbol, prev_ref_index, prev_hyp_index, ref_index,
                     hyp_index))
    ...
    if verbose_level > 2:
        for ref_index in range(ref_len + 1):
            for hyp_index in range(hyp_len + 1):
                print("{0} ".format(H[ref_index][hyp_index]),
                      end='',
                      file=sys.stderr)
...
def get_ctm_edits(alignment_output,
                  ctm_array,
                  eps_symbol="<eps>",
                  oov_word=None,
                  symbol_table=None):
    ...
    for (ref_word, hyp_word, ref_prev_i, hyp_prev_i, ref_i,
         hyp_i) in alignment_output:
        try:
            ctm_pos = hyp_prev_i
            ...
            if hyp_prev_i == hyp_i:
                ...
                edit_type = get_edit_type(hyp_word=eps_symbol,
                                          ref_word=ref_word,
                                          duration=0.0,
                ...
                if hyp_word == eps_symbol and ref_word != eps_symbol:
                    ...
                    edit_type = get_edit_type(hyp_word=hyp_word,
                                              ref_word=ref_word,
                                              duration=ctm_line[1],
                                              eps_symbol=eps_symbol,
                                              oov_word=oov_word,
            ...
            logger.error(
                ...
                "edits@{edits_pos} = {0}, ctm@{ctm_pos} = {1}".format(
                    ("NONE"
                     if ali_pos >= ali_len else alignment_output[ali_pos]),
...
def run(args):
    ...
    if args.hyp_format == "Text":
        ...
        hyp_lines = {
            key: value
            for (key,
                 value) in read_ctm(args.hyp_in_file, file_and_channel2reco)
        }
    ...
    for reco, ref_text in read_text(args.ref_in_file):
        try:
            ...
            if args.hyp_format == "CTM":
                ctm_edits = get_ctm_edits(output,
                                          hyp_lines[reco],
                                          eps_symbol=args.eps_symbol,
