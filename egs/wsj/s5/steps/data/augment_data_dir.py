...
def check_args(args):
    ...
    if args.utt_suffix is None and args.utt_prefix is None:
        ...
        raise Exception(
            "Trying to add both prefix and suffix. Choose either of them")
    ...
    if args.bg_noise_dir is None and args.fg_noise_dir is None:
        raise Exception(
            "Either --fg-noise-dir or --bg-noise-dir must be specified")
    return args


def get_noise_list(noise_wav_scp_filename):
    noise_wav_scp_file = open(
        noise_wav_scp_filename, 'r', encoding='utf-8').readlines()
    noise_wavs = {}
    noise_utts = []
...
def augment_wav(utt, wav, dur, fg_snr_opts, bg_snr_opts, fg_noise_utts,
    ...
    start_times_str = "--start-times='" + \
        ",".join([str(i) for i in start_times]) + "'"
...
def main():
    ...
    wav_scp_file = open(
        input_dir + "/wav.scp", 'r', encoding='utf-8').readlines()
