...
def CheckArgs(args):
    ...
    if not args.relu_dim is None:
        ...
        if (args.nonlin_input_dim < args.nonlin_output_dim) or (args.nonlin_input_dim % args.nonlin_output_dim != 0):
            raise Exception("Invalid --pnorm-input-dim {0} and --pnorm-output-dim {1}".format(
...
def ParseCnnString(cnn_param_string_list):
    ...
    for cl in range(0, len(cnn_param_string_list)):
        cnn_args.append(cnn_parser.parse_args(
            shlex.split(cnn_param_string_list[cl])))
...
def ParseSpliceString(splice_indexes):
    ...
    try:
        ...
        raise Exception("invalid splice-indexes argument " +
...
def MakeConfigs(config_dir, splice_indexes_string,
    ...
    if not nonlin_output_dim is None:
        ...
        nonlin_output_dims = [int(round(
            nonlin_output_dim_init * pow(factor, i))) for i in range(0, num_hidden_layers)]
        assert(nonlin_output_dims[-1] >= nonlin_output_dim_final - 1 and nonlin_output_dims[-1]
               <= nonlin_output_dim_final + 1)  # due to rounding error
        ...
        if xent_separate_forward_affine and i == num_hidden_layers - 1:
            ...
            if nonlin_type == "relu":
                prev_layer_output = nodes.AddAffRelNormLayer(config_lines, "Tdnn_{0}".format(i),
                                                             ...
                                                             norm_target_rms=1.0 if i < num_hidden_layers -
                                                             1 else final_layer_normalize_target,
                                                             self_repair_scale=self_repair_scale,
                                                             max_change_per_component=max_change_per_component)
            elif nonlin_type == "pnorm":
    ...
    print('include_log_softmax=' +
          ('true' if include_log_softmax else 'false'), file=f)
