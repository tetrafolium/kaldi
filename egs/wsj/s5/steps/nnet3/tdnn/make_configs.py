...
def GetArgs():
    ...
    feat_group.add_argument("--feat-dim",
                            type=int,
    ...
    ivector_group.add_argument("--ivector-dim",
                               type=int,
                               help="iVector dimension, e.g. 100",
                               default=0)
    ...
    parser.add_argument(
        ...
        dest="cnn_layer",
        help=
        "CNN parameters at each CNN layer, e.g. --filt-x-dim=3 --filt-y-dim=8 "
    ...
    parser.add_argument(
        ...
        help=
        "The factor used for determining the liftering vector in the production of MFCC. "
    ...
    parser.add_argument("--include-log-softmax",
                        type=str,
                        action=common_lib.StrToBoolAction,
    ...
    parser.add_argument(
        ...
        type=float,
        help=
        "For chain models, if nonzero, add a separate output for cross-entropy "
    ...
    parser.add_argument(
        ...
        action=common_lib.StrToBoolAction,
        help=
        "if using --xent-regularize, gives it separate last-but-one weight matrix",
        default=False,
        choices=["false", "true"])
    parser.add_argument(
        ...
        type=float,
        help=
        "RMS target for final layer (set to <1 if final layer learns too fast",
        default=1.0)
    parser.add_argument(
        "--max-change-per-component",
        type=float,
        help=
        "Enforces per-component max change (except for the final affine layer). "
    ...
    parser.add_argument("--pnorm-input-dim",
                        type=int,
                        help="input dimension to p-norm nonlinearities")
    parser.add_argument("--pnorm-output-dim",
                        type=int,
    ...
    relu_dim_group.add_argument(
        ...
        type=int,
        help=
        "dimension of the last ReLU nonlinearity layer. Dimensions increase geometrically from the first through the last ReLU layer.",
        default=None)
    parser.add_argument(
        "--relu-dim-init",
        type=int,
        help=
        "dimension of the first ReLU nonlinearity layer. Dimensions increase geometrically from the first through the last ReLU layer.",
        default=None)
    ...
    parser.add_argument(
        ...
        type=float,
        help=
        "A non-zero value activates the self-repair mechanism in the sigmoid and tanh non-linearities of the LSTM",
        default=None)

    parser.add_argument("--use-presoftmax-prior-scale",
                        type=str,
                        action=common_lib.StrToBoolAction,
...
def AddConvMaxpLayer(config_lines, name, input, args):
    if '3d-dim' not in input:
        raise Exception(
            "The input to AddConvMaxpLayer() needs '3d-dim' parameters.")

    input = nodes.AddConvolutionLayer(config_lines, name, input,
                                      input['3d-dim'][0], input['3d-dim'][1],
                                      input['3d-dim'][2], args.filt_x_dim,
                                      args.filt_y_dim, args.filt_x_step,
                                      args.filt_y_step, args.num_filters,
                                      input['vectorization'])

    if args.pool_x_size > 1 or args.pool_y_size > 1 or args.pool_z_size > 1:
        input = nodes.AddMaxpoolingLayer(
            config_lines, name, input, input['3d-dim'][0], input['3d-dim'][1],
            input['3d-dim'][2], args.pool_x_size, args.pool_y_size,
            args.pool_z_size, args.pool_x_step, args.pool_y_step,
            args.pool_z_step)
...
def AddCnnLayers(config_lines,
    ...
    common_lib.write_idct_matrix(feat_dim, cepstral_lifter,
    ...
    prev_layer_output = nodes.AddFixedAffineLayer(
        config_lines, "Idct", prev_layer_output,
        config_dir.strip() + '/idct.mat')

    list = [('Offset({0}, {1})'.format(prev_layer_output['descriptor'], n)
             if n != 0 else prev_layer_output['descriptor'])
    ...
    for cl in range(0, num_cnn_layers):
        prev_layer_output = AddConvMaxpLayer(config_lines, "L{0}".format(cl),
                                             prev_layer_output, cnn_args[cl])
    ...
    if ivector_dim > 0:
        ...
        iv_layer_output = nodes.AddAffineLayer(config_lines, "ivector",
                                               iv_layer_output, ivector_dim,
                                               "")
        prev_layer_output['descriptor'] = 'Append({0}, {1})'.format(
            prev_layer_output['descriptor'], iv_layer_output['descriptor'])
        prev_layer_output['dimension'] = prev_layer_output['dimension'] + \
            iv_layer_output['dimension']
...
def ParseCnnString(cnn_param_string_list):
    ...
    for cl in range(0, len(cnn_param_string_list)):
        cnn_args.append(
            cnn_parser.parse_args(shlex.split(cnn_param_string_list[cl])))
...
def ParseSpliceString(splice_indexes):
    ...
    try:
        ...
        raise Exception("invalid splice-indexes argument " + splice_indexes +
                        str(e))
...
def MakeConfigs(
    config_dir, splice_indexes_string, cnn_layer, cnn_bottleneck_dim,
    cepstral_lifter, feat_dim, ivector_dim, num_targets, add_lda, nonlin_type,
    nonlin_input_dim, nonlin_output_dim, subset_dim, nonlin_output_dim_init,
    nonlin_output_dim_final, use_presoftmax_prior_scale,
    final_layer_normalize_target, include_log_softmax, add_final_sigmoid,
    xent_regularize, xent_separate_forward_affine, self_repair_scale,
    max_change_per_component, max_change_per_component_final, objective_type):
    ...
    input_dim = len(
        parsed_splice_output['splice_indexes'][0]) + feat_dim + ivector_dim
    ...
    config_files = {}
    prev_layer_output = nodes.AddInputLayer(config_lines, feat_dim,
                                            splice_indexes[0], ivector_dim)
    ...
    if cnn_layer is not None:
        prev_layer_output = AddCnnLayers(config_lines, cnn_layer,
                                         cnn_bottleneck_dim, cepstral_lifter,
                                         config_dir, feat_dim,
                                         splice_indexes[0], ivector_dim)
    ...
    if not nonlin_output_dim is None:
        ...
        factor = pow(
            float(nonlin_output_dim_final) / nonlin_output_dim_init, 1.0 /
            (num_hidden_layers - 1)) if num_hidden_layers > 1 else 1
        nonlin_output_dims = [
            int(round(nonlin_output_dim_init * pow(factor, i)))
            for i in range(0, num_hidden_layers)
        ]
        assert (nonlin_output_dims[-1] >= nonlin_output_dim_final - 1
                and nonlin_output_dims[-1] <= nonlin_output_dim_final + 1
                )  # due to rounding error
        # It ensures that the dim of the last hidden layer is exactly the same as what is specified
        nonlin_output_dims[-1] = nonlin_output_dim_final
    ...
    for i in range(0, num_hidden_layers):
        ...
        if not (len(splice_indexes[i]) == 1 and splice_indexes[i][0] == 0):
            ...
            prev_layer_output = {
                'descriptor':
                "Append({0})".format(" , ".join(appended_descriptors)),
                'dimension':
                appended_dimension
            }
        else:
        ...
        if xent_separate_forward_affine and i == num_hidden_layers - 1:
            ...
            nodes.AddFinalLayer(
                ...
                num_targets,
                ng_affine_options=
                " param-stddev=0 bias-stddev=0 learning-rate-factor={0} ".
                format(0.5 / xent_regularize),
                max_change_per_component=max_change_per_component_final,
                use_presoftmax_prior_scale=use_presoftmax_prior_scale,
                prior_scale_file=prior_scale_file,
            ...
            if nonlin_type == "relu":
                prev_layer_output = nodes.AddAffRelNormLayer(
                    ...
                    norm_target_rms=1.0 if i < num_hidden_layers - 1 else
        ...
        config_files['{0}/layer{1}.config'.format(config_dir,
                                                  i + 1)] = config_lines
        config_lines = {'components': [], 'component-nodes': []}
    ...
    print('include_log_softmax=' +
          ('true' if include_log_softmax else 'false'),
          file=f)
