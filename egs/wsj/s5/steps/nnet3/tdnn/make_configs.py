...
def AddConvMaxpLayer(config_lines, name, input, args):
    ...
    input = nodes.AddConvolutionLayer(
        ...
        input['3d-dim'][2], args.filt_x_dim, args.filt_y_dim, args.filt_x_step,
        args.filt_y_step, args.num_filters, input['vectorization'])
...
def AddCnnLayers(config_lines,
    ...
    if cnn_bottleneck_dim > 0:
        prev_layer_output = nodes.AddAffineLayer(
            config_lines, "cnn-bottleneck", prev_layer_output,
            cnn_bottleneck_dim, "")
...
def MakeConfigs(config_dir, splice_indexes_string, cnn_layer,
                cnn_bottleneck_dim, cepstral_lifter, feat_dim, ivector_dim,
                num_targets, add_lda, nonlin_type, nonlin_input_dim,
                nonlin_output_dim, subset_dim, nonlin_output_dim_init,
                nonlin_output_dim_final, use_presoftmax_prior_scale,
                final_layer_normalize_target, include_log_softmax,
    ...
    if cnn_layer is not None:
        prev_layer_output = AddCnnLayers(
            ...
            config_dir, feat_dim, splice_indexes[0], ivector_dim)
    ...
    print(
        'include_log_softmax=' + ('true' if include_log_softmax else 'false'),
        file=f)
