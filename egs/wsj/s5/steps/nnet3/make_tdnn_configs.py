...
print('output-node name=output input=Append({0})'.format(", ".join(list)),
      file=f)
f.close()
...
for l in range(1, num_hidden_layers + 1):
    ...
    if l == 1:
        print(
            ...
            file=f)
    cur_dim = (nonlin_output_dim *
               len(splice_array[l - 1]) if l > 1 else input_dim)
    ...
    if l == 1:
        ...
        print('component-node name=lda component=lda input={0}'.format(
            orig_input),
              file=f)
        cur_input = 'lda'
    else:
