...
print(
    'output-node name=output input=Append({0})'.format(", ".join(list)), file=f)
f.close()
...
for l in range(1, num_hidden_layers + 1):
    ...
    if l == 1:
        splices = [('Offset(input, {0})'.format(
            n) if n != 0 else 'input') for n in splice_array[l-1]]
        if args.ivector_dim > 0:
