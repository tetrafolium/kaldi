...
def get_args():
    ...
    parser.add_argument(
        ...
        default=None,
        help=
        "Number of archives to split the data into. (Note: in reality they are not "
    ...
    parser.add_argument("egs_scp_lists",
                        nargs='+',
...
def process_multilingual_egs(args):
    ...
    with open("{0}/info/{1}num_tasks".format(args.egs_dir, args.egs_prefix),
              "w") as fh:
    ...
    with open("{0}/info/{1}num_archives".format(args.egs_dir, args.egs_prefix),
              "w") as fh:
    ...
    logger.info(
        ...
        "{}, and block-size is {}.".format(
            int(round(tot_num_egs / num_archives / args.block_size)),
            args.block_size))
    for lang in range(num_langs):
        blocks_per_archive_this_lang = lang_to_num_examples[lang] / \
            num_archives / args.block_size
        warning = ""
        ...
        logger.info(
            ...
            "{}".format(lang,
                        ...
                        blocks_per_archive_this_lang, warning))

    in_scp_file_handles = [
        open(scp_lists[lang], 'r') for lang in range(num_langs)
    ]
    ...
    for archive_index in range(num_archives + 1):
        ...
        if not last_round:
            ...
            logger.info("Generating archive {} containing {} blocks...".format(
            ...
            logger.info(
                "Writing all the {} remaining egs to the last archive...".
                format(num_remaining_egs))

        out_scp_file_handle = open(
            '{0}/{1}{2}.scp'.format(args.egs_dir, args.egs_prefix,
                                    archive_index + 1),
            'a' if last_round else 'w')
        eg_to_output_file_handle = open(
            "{0}/{1}output.{2}.ark".format(args.egs_dir, args.egs_prefix,
                                           archive_index + 1),
            'a' if last_round else 'w')
        eg_to_weight_file_handle = open(
            "{0}/{1}weight.{2}.ark".format(args.egs_dir, args.egs_prefix,
                                           archive_index + 1),
            'a' if last_round else 'w')
        ...
        for block_index in range(num_blocks_this_archive):
            ...
            remaining_proportions = [
                float(remain) / tot for remain, tot in zip(
                    lang_to_num_remaining_egs, lang_to_num_examples)
            ]
            lang_index, max_proportion = max(enumerate(remaining_proportions),
                                             key=lambda a: a[1])

            # Read 'block_size' examples from the selected lang and write them to the current output scp file:
            example_lines = read_lines(in_scp_file_handles[lang_index],
                                       args.block_size)
            for eg_line in example_lines:
                eg_id = eg_line.split()[0]
                print(eg_line, file=out_scp_file_handle)
                print("{0} output-{1}".format(eg_id, lang_index),
                      file=eg_to_output_file_handle)
                print("{0} {1}".format(eg_id, lang2weight[lang_index]),
