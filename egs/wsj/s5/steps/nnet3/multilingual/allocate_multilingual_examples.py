...
def process_multilingual_egs(args):
    ...
    in_scp_file_handles = [open(scp_lists[lang], 'r')
                           for lang in range(num_langs)]
    ...
    for archive_index in range(num_archives + 1):
        ...
        if not last_round:
            ...
            logger.info("Writing all the {} remaining egs to the last archive...".format(
                num_remaining_egs))
        ...
        for block_index in range(num_blocks_this_archive):
            ...
            remaining_proportions = [float(
                remain) / tot for remain, tot in zip(lang_to_num_remaining_egs, lang_to_num_examples)]
            ...
            example_lines = read_lines(
                in_scp_file_handles[lang_index], args.block_size)
            for eg_line in example_lines:
                ...
                print("{0} {1}".format(
                    eg_id, lang2weight[lang_index]), file=eg_to_weight_file_handle)
