...
def GetArgs():
    ...
    parser.add_argument("--recurrent-projection-dim",
                        type=int,
                        help="dimension of recurrent projection")
    parser.add_argument("--non-recurrent-projection-dim",
                        type=int,
    ...
    parser.add_argument(
        ...
        type=str,
        help=
        "options to be supplied to NaturalGradientPerElementScaleComponent",
        default="")
    parser.add_argument(
        "--ng-affine-options",
        type=str,
...
def CheckArgs(args):
    ...
    if args.lstm_delay is None:
        ...
        try:
            ...
            sys.exit(
                "--lstm-delay has incorrect format value. Provided value is '{0}'"
                .format(args.lstm_delay))
    ...
    try:
        for i in range(len(split1)):
            ...
            if not indexes == sorted(indexes):
                raise ValueError(
                    "elements of --splice-indexes must be sorted: " +
                    splice_indexes)
...
def ParseLstmDelayString(lstm_delay):
    ...
    try:
        for i in range(len(split1)):
            ...
            if len(indexes) < 1:
                raise ValueError(
                    "invalid --lstm-delay argument, too-short element: " +
                    lstm_delay)
        ...
        raise ValueError("invalid --lstm-delay argument " + lstm_delay +
                         str(e))

    return lstm_delay_array


def MakeConfigs(config_dir, feat_dim, ivector_dim, num_targets, splice_indexes,
                lstm_delay, cell_dim, hidden_dim, recurrent_projection_dim,
                non_recurrent_projection_dim, num_lstm_layers,
                num_hidden_layers, norm_based_clipping, clipping_threshold,
                zeroing_threshold, zeroing_interval,
                ng_per_element_scale_options, ng_affine_options, label_delay,
...
def ProcessSpliceIndexes(config_dir, splice_indexes, label_delay,
                         num_lstm_layers):
    parsed_splice_output = ParseSpliceString(splice_indexes.strip(),
                                             label_delay)
...
def Main():
    ...
    [left_context, right_context, num_hidden_layers, splice_indexes
     ] = ProcessSpliceIndexes(args.config_dir, args.splice_indexes,
