...
def read_int(s, pos):
    ...
    try:
        ...
        print(
            "{0}: at file position {1}, expected int but got {1}".format(tok).
            format(sys.argv[0], orig_pos, tok),
            file=sys.stderr)
        return (None, pos)
    return (i, pos)
...
def read_vector(s, pos):
    ...
    if tok != '[':
        print(
            "{0}: at file position {1}, expected vector but got {1}".format(
...
def read_matrix(s, pos):
    ...
    if tok != '[':
        print(
            "{0}: at file position {1}, expected matrix but got {1}".format(
                tok).format(sys.argv[0], pos, tok),
            file=sys.stderr)
        return (None, pos)
...
def read_model(filename):
    ...
    for c in range(num_components):
        ...
        if component != None:
            ...
            print(
                "{0}: error reading component with name {1} at position {2}".
        ...
        if raw_component_type in {'Linear', 'Affine', 'NaturalGradientAffine'}:
            ...
            if size % 3 == 0:
                ...
                c['col-norms-3'] = np.sqrt(
                    np.sum(
                        np.power(c['col-norms'], 2).reshape(3, size / 3),
                        axis=0))
...
def compute_progress(model1, model2):
    ...
    for component_name in model1:
        ...
        if raw_component_type in {'Linear', 'Affine', 'NaturalGradientAffine'}:
            ...
            epsilon = 1.0e-20
            if 'row-norms' in c1:
                c1['rel-row-change'] = c1['row-change'] / \
                    (c1['row-norms'] + epsilon)
            if 'col-norms' in c1:
                c1['rel-col-change'] = c1['col-change'] / \
                    (c1['col-norms'] + epsilon)
...
if __name__ == '__main__':
    ...
    if len(sys.argv) != 3:
        print(
            "Usage: {0} <nnet3-model-in> <pickled-model-out>".format(
                sys.argv[0]),
            file=sys.stderr)
        sys.exit(1)
    ...
    if m != None:
        try:
            ...
            print(
                "{0}: error writing to {1}".format(sys.argv[2]),
                file=sys.stderr)
