...
def get_args():
    ...
    if ((args.comparison_dir is not None and len(args.comparison_dir) > 6)
            or (args.exp_dir is not None and len(args.exp_dir) > 7)):
        raise Exception(
    ...
    if args.is_chain and args.is_rnnlm:
        raise Exception(
            "Options --is-chain and --is-rnnlm cannot be both true.")
    return args
...
"""
    ...
    def compile(self):
        ...
        try:
            common_lib.execute_command("pdflatex -interaction=batchmode "
...
def generate_acc_logprob_plots(exp_dir,
                               output_dir,
                               plot,
                               key='accuracy',
    ...
    for dir in dirs:
        [report, times,
         data] = log_parse.generate_acc_logprob_report(dir, key, output_name)
        if index == 0:
...
g_lstm_gate = [
    'i_t_sigmoid', 'f_t_sigmoid', 'c_t_tanh', 'o_t_sigmoid', 'm_t_tanh'
]
...
def insert_a_column_legend(legend_handle, legend_label, lp, mp, hp, dir,
                           prefix_length, column_index):
...
def plot_a_nonlin_component(fig,
                            dirs,
            ...
            if stats_per_dir[exp_dir][component_name][
                ...
                for i in range(0, 5):
                    ...
                    lgd = plot_a_nonlin_component(
                        ...
                        component_type, start_iter, i, with_oderiv)
                    fig.suptitle(
                ...
                lgd = plot_a_nonlin_component(
                    fig, dirs, stat_tables_per_component_per_dir,
                    component_name, common_prefix, prefix_length,
                    component_type, start_iter, 0, with_oderiv)
                if with_oderiv:
                    fig.suptitle(
...
def generate_clipped_proportion_plots(exp_dir,
                                      output_dir,
                                      plot,
                                      comparison_dir=None,
                                      start_iter=1,
    ...
    if plot:
        ...
        for dir in dirs:
            try:
                component_names = set(
                    stats_per_dir[dir]['cp_per_iter_per_component'])
...
def generate_plots(exp_dir,
    ...
    for (output_name, objective_type) in output_names:
        if objective_type == "linear":
            ...
            generate_acc_logprob_plots(exp_dir,
                                       output_dir,
                                       g_plot,
                                       key='accuracy',
                                       file_basename='accuracy',
            ...
            logger.info("Generating log-likelihood plots for '%s'",
                        output_name)
            generate_acc_logprob_plots(exp_dir,
                                       output_dir,
                                       g_plot,
            ...
            logger.info("Generating log-probability plots for '%s'",
                        output_name)
            generate_acc_logprob_plots(exp_dir,
                                       output_dir,
                                       g_plot,
            ...
            logger.info("Generating RNNLM objective plots for '%s'",
                        output_name)
            generate_acc_logprob_plots(exp_dir,
                                       output_dir,
                                       g_plot,
            ...
            logger.info("Generating %s objective plots for '%s'",
                        objective_type, output_name)
            generate_acc_logprob_plots(exp_dir,
                                       output_dir,
                                       g_plot,
                                       key='objective',
    ...
    generate_nonlin_stats_plots(exp_dir,
                                output_dir,
                                g_plot,
                                comparison_dir=comparison_dir,
                                start_iter=start_iter,
    ...
    generate_clipped_proportion_plots(exp_dir,
                                      output_dir,
                                      g_plot,
                                      comparison_dir=comparison_dir,
                                      start_iter=start_iter,
    ...
    generate_parameter_diff_plots(exp_dir,
                                  output_dir,
                                  g_plot,
                                  comparison_dir=comparison_dir,
                                  start_iter=start_iter,
    ...
    if g_plot and latex_report is not None:
        ...
        if has_compiled:
            logger.info(
                "Report file %s/report.pdf has been generated successfully.",
                output_dir)
