...
def train(args, run_opts):
    ...
    # Check files
    files = [
        '{0}/feats.scp'.format(args.feat_dir), '{0}/fst.1.scp'.format(
            args.tree_dir), '{0}/final.mdl'.format(args.tree_dir),
        '{0}/tree'.format(args.tree_dir), '{0}/phone_lm.fst'.format(
            args.tree_dir), '{0}/num_jobs'.format(args.tree_dir)
    ]
    for file in files:
        ...
        common_lib.execute_command(
            """steps/nnet3/chain/e2e/get_egs_e2e.sh {egs_opts} \
                ...
                ivector_dir=(args.online_ivector_dir
                             if args.online_ivector_dir is not None else ''),
