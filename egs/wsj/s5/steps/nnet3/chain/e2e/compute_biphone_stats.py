...
parser.add_argument('--shared-phones',
                    type=str,
                    choices=['true', 'false'],
                    default='true',
...
print('Loaded {} phone-sets containing {} phones.'.format(
    len(phone_sets), len(phones)),
      file=sys.stderr)
...
for line in sys.stdin:
    ...
    key = line[0]
    line_phones = line[1:]
    for pair in zip([0] + line_phones,
                    line_phones):  # 0 is for the no left-context case
        ...
        mono_counts[pair[
            1]] = 1 if pair[1] not in mono_counts else mono_counts[pair[1]] + 1

for phone1 in [0] + phones:
    for phone2 in phones:
        pair = (phone1, phone2)
        shared_pair = ((phone_to_shard_phone[pair[0]],
                        phone_to_shard_phone[pair[1]])
                       if args.shared_phones else pair)
        count = biphone_counts[
            shared_pair] if shared_pair in biphone_counts else 0
        if count != 0:
