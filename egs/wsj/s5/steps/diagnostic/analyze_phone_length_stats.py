...
try:
    ...
    for p in phone_int2text.keys():
        if p > 0 and not p in nonsilence:
            this_count = sum(
                [l * c for l, c in phone_lengths['all'][p].items()])
...
for boundary_type in 'begin', 'end':
    ...
    frequency_percentage = sum(
        opt_sil_lengths.values()) * 100.0 / num_utterances
...
for boundary_type in 'begin', 'end', 'all':
    ...
    for phone, lengths in sorted(phone_to_lengths.items(),
...
total_frames['internal'] = total_frames['all'] - \
    total_frames['begin'] - total_frames['end']
total_phones['internal'] = total_phones['all'] - \
    total_phones['begin'] - total_phones['end']
