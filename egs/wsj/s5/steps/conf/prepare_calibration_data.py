...
parser.add_option(
    "--conf-targets",
    help=
    "Targets file for logistic regression (no targets generated if '') [default %default]",
    default='')
parser.add_option(
    "--conf-feats",
    ...
    with open(o.conf_targets, 'w') as f:
        for (utt, chan, beg, dur, wrd_id, conf, score_tag) in ctm:
            ...
            if not word_filter[wrd_id]:
                continue
            # Build the key,
            key = "%s^%s^%s^%s^%s,%s,%s" % (utt, chan, beg, dur, wrd_id, conf,
                                            score_tag)
            # Build the target,
...
with open(o.conf_feats, 'w') as f:
    for (utt, chan, beg, dur, wrd_id, conf, score_tag) in ctm:
        ...
        if o.lattice_depth != '':
            depth_slice = depths[utt][int(round(100.0 * float(beg))):int(
                round(100.0 * (float(beg) + float(dur))))]
            log_avg_depth = math.log(
                float(sum(depth_slice)) / len(depth_slice))
            feats += [log_avg_depth]
