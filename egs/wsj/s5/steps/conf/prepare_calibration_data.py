...
parser.add_option("--conf-targets",
                  help="Targets file for logistic regression (no targets generated if '') [default %default]", default='')
parser.add_option(
    "--conf-feats", help="Feature file for logistic regression. [default %default]", default='')
parser.add_option("--lattice-depth",
                  help="Per-frame lattice depths, ascii-ark (optional). [default %default]", default='')
...
word_filter = {wrd_id: bool(int(filter)) for (
    wrd, wrd_id, filter, length, other_feats) in word_feats}
# Prepare the lenght dict,
word_length = {wrd_id: float(length) for (
    wrd, wrd_id, filter, length, other_feats) in word_feats}
# Prepare other_feats dict,
other_feats = {wrd_id: other_feats.strip()
...
if o.conf_targets != '':
    with open(o.conf_targets, 'w') as f:
        for (utt, chan, beg, dur, wrd_id, conf, score_tag) in ctm:
            ...
            # Build the key,
            key = "%s^%s^%s^%s^%s,%s,%s" % (
                utt, chan, beg, dur, wrd_id, conf, score_tag)
            # Build the target,
...
with open(o.conf_feats, 'w') as f:
    for (utt, chan, beg, dur, wrd_id, conf, score_tag) in ctm:
        ...
        logit = math.log(float(conf)+damper) - \
            math.log(1.0 - float(conf)+damper)
        # - log of word-length,
        # i.e. number of phones in a word,
        log_word_length = math.log(word_length[wrd_id])
        ...
        if o.lattice_depth != '':
            depth_slice = depths[utt][int(
                round(100.0*float(beg))):int(round(100.0*(float(beg)+float(dur))))]
