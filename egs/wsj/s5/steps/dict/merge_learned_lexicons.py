...
def GetArgs():
    parser = argparse.ArgumentParser(
        description=
        "Convert a learned lexicon produced by steps/dict/select_prons_greedy.sh"
    ...
    parser.add_argument(
        ...
        type=str,
        help=
        "File containing word-pronunciation statistics obtained from lattices; "
    ...
    parser.add_argument(
        ...
        "Each line must be <word> <phones>")
    parser.add_argument("learned_lexicon",
...
def WriteEditsAndSummary(args, learned_lexicon, ref_lexicon, pd_lexicon,
                         g2p_lexicon, counts, stats, stats_summed):
    ...
    for word, count in words_to_edit_sorted:
        ...
        for pron in ref_lexicon[word]:
            if pron not in learned_lexicon[word]:
                learned_prons.append(
                    ('R', 'N', stats_summed[(word, pron)], pron))
        learned_prons_sorted = sorted(learned_prons,
    ...
    print(
        ...
        file=sys.stderr)
    print(
        "We have acoustic evidence for {} out of {} in-vocab (w.r.t the reference lexicon) words from the acoustic training data."
        .format(num_ivs, len(ref_lexicon)),
        file=sys.stderr)
    print("  Among those frequent words whose counts in the training text > ",
          threshold,
          ":",
          file=sys.stderr)
    ...
    print(
        'We have acoustic evidence for {} out of {} OOV (w.r.t the reference lexicon) words from the acoustic training data.'
        .format(num_oovs_with_acoustic_evidence, num_oovs),
        file=sys.stderr)
    print('  Among those words whose counts in the training text > {}:'.format(
        threshold),
          file=sys.stderr)
...
def Main():
    ...
    WriteEditsAndSummary(args, learned_lexicon, ref_lexicon, pd_lexicon,
                         g2p_lexicon, counts, stats, stats_summed)
