...
def CheckArgs(args):
    ...
    for i in range(0, 3):
        if float(prior_mean[i]) <= 0 or float(prior_mean[i]) >= 1:
            raise Exception(
                'Dirichlet prior mean', prior_mean[i], 'is invalid, it must be between 0 and 1.')
    args.prior_mean = [float(prior_mean[0]), float(
        prior_mean[1]), float(prior_mean[2])]
...
def ComputePriorCounts(args, counts, ref_lexicon, g2p_lexicon, phonetic_decoding_lexicon):
    ...
    for word in counts:
        prior_mean = [args.prior_mean[0],
                      args.prior_mean[1], args.prior_mean[2]]
        ...
        try:
            ...
            print('WARNING: word {} appears in train_counts but not in any lexicon.'.format(
    ...
    for word, entry in posteriors.items():
        ...
        for pron, count in entry:
            ...
            if word in g2p_lexicon and pron in g2p_lexicon[word]:
                ...
                source = 'P'
            print(word, source, "%3.2f" %
                  post, pron, file=args.pron_posteriors_handle)
        del entry[:]
...
def SelectPronsBayesian(args, counts, posteriors, ref_lexicon, g2p_lexicon, phonetic_decoding_lexicon):
    ...
    print('---------------------------------------------------------------------------------------------------', file=sys.stderr)
    print('Num. words in the learned lexicon: {0} num. selected prons: {1}'.format(
        len(learned_lexicon), num_prons_tot), file=sys.stderr)
...
def WriteEditsAndSummary(args, learned_lexicon, ref_lexicon, phonetic_decoding_lexicon, g2p_lexicon, counts, stats):
    ...
    print("# Note: This file contains pronunciation info for words who have candidate"
          ...
          ", sorted by their counts in acoustic training data, ", file=args.ref_lexicon_edits_handle)
    ...
    for word, count in words_to_edit_sorted:
        print("------------", word, "%2.1f" %
              count, "--------------", file=args.ref_lexicon_edits_handle)
        for pron in learned_lexicon[word]:
        ...
        for pron in ref_lexicon[word]:
            if pron not in learned_lexicon[word]:
                ...
                print('R  | N |  {:.2f} | {} '.format(
                    soft_count, pron), file=args.ref_lexicon_edits_handle)
    print("Here are the words whose reference pron candidates were all declined",
          words[0]['100'], file=sys.stderr)
    print("-------------------------------------------------Summary------------------------------------------", file=sys.stderr)
    print("In the learned lexicon, out of those", len(ref_lexicon),
          "words from the vocab of the reference lexicon:", file=sys.stderr)
    print("  For those frequent words whose counts in the training text > ",
          threshold, ":", file=sys.stderr)
    num_freq_ivs_from_all_sources = len(
        words[0]['111']) + len(words[0]['110']) + len(words[0]['011'])
    num_freq_ivs_from_g2p_or_phonetic_decoding = len(
        words[0]['101']) + len(words[0]['001']) + len(words[0]['100'])
    num_freq_ivs_from_ref = len(words[0]['010'])
    num_infreq_ivs_from_all_sources = len(
        words[1]['111']) + len(words[1]['110']) + len(words[1]['011'])
    num_infreq_ivs_from_g2p_or_phonetic_decoding = len(
        words[1]['101']) + len(words[1]['001']) + len(words[1]['100'])
    ...
    print(' {} words\' selected prons came from the reference lexicon only.'.format(
        num_freq_ivs_from_ref), file=sys.stderr)
    ...
    print(' {} words\' selected prons came from the reference lexicon only.'.format(
        num_infreq_ivs_from_ref), file=sys.stderr)
    print("---------------------------------------------------------------------------------------------------", file=sys.stderr)
    ...
    print('  In the learned lexicon, out of those {} OOV words (w.r.t the reference lexicon):'.format(
        num_oovs), file=sys.stderr)
    print('  For those words whose counts in the training text > {}:'.format(
        threshold), file=sys.stderr)
    print('    {} words\' selected prons came from G2P and phonetic-decoding.'.format(
        num_freq_oovs_from_both_sources), file=sys.stderr)
    print('    {} words\' selected prons came from phonetic decoding only.'.format(
        num_freq_oovs_from_phonetic_decoding), file=sys.stderr)
    print('    {} words\' selected prons came from G2P only.'.format(
        num_freq_oovs_from_g2p), file=sys.stderr)
    print('  For those words whose counts in the training text <= {}:'.format(
        threshold), file=sys.stderr)
    print('    {} words\' selected prons came from G2P and phonetic-decoding.'.format(
        num_infreq_oovs_from_both_sources), file=sys.stderr)
    print('    {} words\' selected prons came from phonetic decoding only.'.format(
        num_infreq_oovs_from_phonetic_decoding), file=sys.stderr)
    print('    {} words\' selected prons came from G2P only.'.format(
        num_infreq_oovs_from_g2p), file=sys.stderr)
