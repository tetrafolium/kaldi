...
def GetArgs():
    ...
    parser.add_argument(
        ...
        "of the ref. lexicon.")
    parser.add_argument("silence_file",
    ...
    parser.add_argument("g2p_lexicon",
    ...
    parser.add_argument(
        ...
        type=str,
        help=
        "Output file containing posteriors of all candidate prons for each word,"
    ...
    parser.add_argument(
        ...
        type=str,
        help=
        "Output file which is the learned lexicon for words out of the ref. vocab."
    )
    parser.add_argument(
        "ref_lexicon_edits",
...
def CheckArgs(args):
    ...
    args.phonetic_decoding_lexicon_handle = open(
        args.phonetic_decoding_lexicon)
    ...
    for i in range(0, 3):
        if float(prior_mean[i]) <= 0 or float(prior_mean[i]) >= 1:
            raise Exception('Dirichlet prior mean', prior_mean[i],
...
def ComputePriorCounts(args, counts, ref_lexicon, g2p_lexicon,
                       phonetic_decoding_lexicon):
    ...
    for word in counts:
        prior_mean = [
            args.prior_mean[0], args.prior_mean[1], args.prior_mean[2]
        ]
        ...
        try:
            ...
            print(
                'WARNING: word {} appears in train_counts but not in any lexicon.'
                .format(word),
                file=sys.stderr)
...
def ComputePosteriors(args, stats, ref_lexicon, g2p_lexicon,
                      phonetic_decoding_lexicon, prior_counts):
    # This dict stores a list of (pronunciation, posterior)
    posteriors = defaultdict(list)
    ...
    for word, prons in ref_lexicon.items():
        for pron in prons:
            ...
            c = float(prior_counts[word][0]) / \
                len(ref_lexicon[word]) + stats.get((word, pron), 0)
            posteriors[word].append((pron, c))

    for word, prons in g2p_lexicon.items():
        for pron in prons:
            c = float(prior_counts[word][1]) / \
                len(g2p_lexicon[word]) + stats.get((word, pron), 0)
            posteriors[word].append((pron, c))

    for word, prons in phonetic_decoding_lexicon.items():
        for pron in prons:
            c = float(prior_counts[word][2]) / \
                len(phonetic_decoding_lexicon[word]
    ...
    print(
        ...
        'phonetic_decoding'.format(num_prons_from_ref, num_prons_from_g2p,
                                   num_prons_from_phonetic_decoding),
        file=sys.stderr)
    print(
        "---------------------------------------------------------------------------------------------------",
        file=sys.stderr)
    ...
    # This dict stores the pronunciation which has
    ...
    for word, entry in posteriors.items():
        ...
        for pron, count in entry:
            ...
            if word in g2p_lexicon and pron in g2p_lexicon[word]:
                source = 'G'
            elif word in phonetic_decoding_lexicon and pron in phonetic_decoding_lexicon[
                    word]:
                source = 'P'
            print(word,
                  source,
                  "%3.2f" % post,
                  pron,
                  file=args.pron_posteriors_handle)
        del entry[:]
        entry.extend(sorted(new_entry, key=lambda new_entry: new_entry[1]))
    return posteriors


def SelectPronsBayesian(args, counts, posteriors, ref_lexicon, g2p_lexicon,
                        phonetic_decoding_lexicon):
    ...
    for word, entry in posteriors.items():
        ...
        last_post = 0.0
        while (
            (num_variants < variants_counts and post_tot < variants_prob_mass)
                or
            (len(entry) > 0 and entry[-1][1] == last_post)):  # this conditions
        ...
        while (num_variants < variants_counts
               and post_tot < args.variants_prob_mass_ref):
            try:
                pron, post = entry.pop()
            except IndexError:
                break
    ...
    print(
        '---------------------------------------------------------------------------------------------------',
        file=sys.stderr)
    print('Num. words in the learned lexicon: {0} num. selected prons: {1}'.
          format(len(learned_lexicon), num_prons_tot),
          file=sys.stderr)
    print(
...
def WriteEditsAndSummary(args, learned_lexicon, ref_lexicon,
                         phonetic_decoding_lexicon, g2p_lexicon, counts,
                         stats):
    ...
    threshold = 3
    # "words" contains four bins, where we
    words = [defaultdict(set) for i in range(4)]
    ...
    print(
        ...
        file=args.ref_lexicon_edits_handle)
    print("# 2nd Col: accepted or not in the learned lexicon (Y/N).",
          file=args.ref_lexicon_edits_handle)
    print(
        "# 3rd Col: soft counts from lattice-alignment (not augmented by prior-counts).",
        file=args.ref_lexicon_edits_handle)
    print("# 4th Col: the pronunciation cadidate.",
          file=args.ref_lexicon_edits_handle)
    ...
    for word in learned_lexicon:
        ...
        for pron in learned_lexicon[word]:
            if word in phonetic_decoding_lexicon and pron in phonetic_decoding_lexicon[
                    word]:
                flags[0] = '1'
                src[(word, pron)] = 'P'
            if word in ref_lexicon and pron in ref_lexicon[word]:
    ...
    words_to_edit_sorted = sorted(words_to_edit,
    ...
    for word, count in words_to_edit_sorted:
        ...
        for pron in ref_lexicon[word]:
            if pron not in learned_lexicon[word]:
                ...
                print('R  | N |  {:.2f} | {} '.format(soft_count, pron),
                      file=args.ref_lexicon_edits_handle)
    print(
    ...
    print(
        "-------------------------------------------------Summary------------------------------------------",
        file=sys.stderr)
    print("In the learned lexicon, out of those",
          len(ref_lexicon),
          "words from the vocab of the reference lexicon:",
          file=sys.stderr)
    print("  For those frequent words whose counts in the training text > ",
          threshold,
          ":",
          file=sys.stderr)
    num_freq_ivs_from_all_sources = len(words[0]['111']) + len(
        words[0]['110']) + len(words[0]['011'])
    num_freq_ivs_from_g2p_or_phonetic_decoding = len(words[0]['101']) + len(
        words[0]['001']) + len(words[0]['100'])
    num_freq_ivs_from_ref = len(words[0]['010'])
    num_infreq_ivs_from_all_sources = len(words[1]['111']) + len(
        words[1]['110']) + len(words[1]['011'])
    num_infreq_ivs_from_g2p_or_phonetic_decoding = len(words[1]['101']) + len(
        words[1]['001']) + len(words[1]['100'])
    num_infreq_ivs_from_ref = len(words[1]['010'])
    print(
        ' {} words\' selected prons came from the reference lexicon, G2P/phonetic-decoding.'
        .format(num_freq_ivs_from_all_sources),
        file=sys.stderr)
    print(
        ' {} words\' selected prons come from G2P/phonetic-decoding-generated.'
        .format(num_freq_ivs_from_g2p_or_phonetic_decoding),
        file=sys.stderr)
    print(' {} words\' selected prons came from the reference lexicon only.'.
          format(num_freq_ivs_from_ref),
          file=sys.stderr)
    print('  For those words whose counts in the training text <= {}:'.format(
        threshold),
          file=sys.stderr)
    print(
        ' {} words\' selected prons came from the reference lexicon, G2P/phonetic-decoding.'
        .format(num_infreq_ivs_from_all_sources),
        file=sys.stderr)
    print(
        ' {} words\' selected prons come from G2P/phonetic-decoding-generated.'
        .format(num_infreq_ivs_from_g2p_or_phonetic_decoding),
        file=sys.stderr)
    print(' {} words\' selected prons came from the reference lexicon only.'.
          format(num_infreq_ivs_from_ref),
          file=sys.stderr)
    print(
        "---------------------------------------------------------------------------------------------------",
        file=sys.stderr)
    ...
    print(
        '  In the learned lexicon, out of those {} OOV words (w.r.t the reference lexicon):'
        .format(num_oovs),
        file=sys.stderr)
    print('  For those words whose counts in the training text > {}:'.format(
        threshold),
          file=sys.stderr)
    print('    {} words\' selected prons came from G2P and phonetic-decoding.'.
          format(num_freq_oovs_from_both_sources),
          file=sys.stderr)
    print('    {} words\' selected prons came from phonetic decoding only.'.
          format(num_freq_oovs_from_phonetic_decoding),
          file=sys.stderr)
    print('    {} words\' selected prons came from G2P only.'.format(
    ...
    print('  For those words whose counts in the training text <= {}:'.format(
        threshold),
          file=sys.stderr)
    print('    {} words\' selected prons came from G2P and phonetic-decoding.'.
          format(num_infreq_oovs_from_both_sources),
          file=sys.stderr)
    print('    {} words\' selected prons came from phonetic decoding only.'.
          format(num_infreq_oovs_from_phonetic_decoding),
          file=sys.stderr)
    print('    {} words\' selected prons came from G2P only.'.format(
...
def Main():
    ...
    prior_counts = ComputePriorCounts(args, counts, ref_lexicon, g2p_lexicon,
                                      phonetic_decoding_lexicon)
    # Compute posteriors, and then select prons to construct the learned lexicon.
    posteriors = ComputePosteriors(args, stats, ref_lexicon, g2p_lexicon,
                                   phonetic_decoding_lexicon, prior_counts)

    # Select prons to construct the learned lexicon.
    learned_lexicon = SelectPronsBayesian(args, counts, posteriors,
                                          ref_lexicon, g2p_lexicon,
                                          phonetic_decoding_lexicon)

    # Write the learned prons for words out of the ref. vocab into learned_lexicon_oov.
    WriteLearnedLexiconOov(learned_lexicon, ref_lexicon,
                           args.learned_lexicon_oov_handle)
    # Edits will be printed into ref_lexicon_edits, and the summary will be printed into stderr.
    WriteEditsAndSummary(args, learned_lexicon, ref_lexicon,
                         phonetic_decoding_lexicon, g2p_lexicon, counts, stats)
