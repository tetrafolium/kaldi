...
def GetArgs():
    ...
    parser.add_argument("silence_phones_file",
...
def SelectPronsGreedy(args,
                      stats,
                      counts,
                      ref_lexicon,
                      g2p_lexicon,
                      pd_lexicon,
                      dianostic_info=False):
    ...
    src = {}
    # Put all selected prons in this dictionary
    ...
    for word in prons:
        ...
        while len(active_indexes) > 1:
            ...
            while abs(log_like - log_like_last) > 1e-7:
                ...
                pron_probs, log_like = OneEMIter(args, word, stats, prons,
                                                 pron_probs, False)
                # the first iteration
                if log_like_last == 1.0 and len(soft_counts_normalized) == 0:
                    soft_counts_normalized = pron_probs
                    if dianostic_info:
                        print("Avg.(over all egs) soft counts: {}".format(
                            soft_counts_normalized))
            if dianostic_info:
                print(
                    "\n Log_like after {} iters of EM: {}, estimated pron_probs: {} \n"
                    .format(num_iters, log_like, pron_probs))
            ...
            for i in active_indexes:
                ...
                pron_probs_mod = [
                    s / sum(pron_probs_mod) for s in pron_probs_mod
                ]
                log_like2 = 1.0
                log_like2_last = -1.0
                num_iters2 = 0
                ...
                if dianostic_info:
                    print(
                        ...
                        " loss in avg. log-likelihood; Num. iters until converging:{}. "
                        .format(prons[word][i], source, loss, num_iters2))
            ...
            if dianostic_info:
                print(
                    "removed pron {}, from source {} with quality score {:.5f}"
                    .format(pron, source, quality_score))
                if (source == 'P' and soft_count > 0.7
                        and len(stats[word]) > 5):
                    print(
                        ...
                        "    rejected if alpha_{pd} is smaller than {}".format(
                            word, len(stats[word]), pron, soft_count,
                            -loss / log_delta,
                    ...
                    if loss_abs > thr:
                        print("    or beta_{pd} is smaller than {}".format(
                            (loss_abs / thr - 1) * len(stats[word])),
                              file=sys.stderr)
                if (source == 'G' and soft_count > 0.7
                        and len(stats[word]) > 5):
                    print(
                        ...
                        "    soft count {} (out of 1) is rejected. It shouldn't have been"
                        "    rejected if alpha_{g2p} is smaller than {} ".
                        format(
                            word, len(stats[word]), pron, soft_count,
                            -loss / log_delta,
...
def Main():
    ...
    learned_lexicon = SelectPronsGreedy(args, stats, counts, ref_lexicon,
                                        g2p_lexicon, pd_lexicon)
