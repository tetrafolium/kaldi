...
def GetArgs():
    parser = argparse.ArgumentParser(
        description=
        "Accumulate statistics from lattice-alignment outputs for lexicon"
...
def GetStatsFromArcInfo(arc_info_file_handle, phone_map_handle):
    ...
    for line in arc_info_file_handle.readlines():
        ...
        stats_unmapped[(word, phones)] = stats_unmapped.get(
            (word, phones), 0) + count
...
def WriteStats(stats, file_handle):
    for word_pron, count in stats.items():
        print(
            '{2} {0} {1}'.format(word_pron[0], word_pron[1], count),
            file=file_handle)
    file_handle.close()
...
def Main():
    args = GetArgs()
    stats = GetStatsFromArcInfo(args.arc_info_file_handle,
