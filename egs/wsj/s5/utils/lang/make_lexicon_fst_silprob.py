        ...
        for line in f:
            ...
            try:
                ...
                print("{0}: error: found bad line '{1}' in silprobs file {1}".
                      ...
                      file=sys.stderr)
                sys.exit(1)
    if (silbeginprob <= 0.0 or silbeginprob > 1.0 or silendcorrection <= 0.0
            or nonsilendcorrection <= 0.0 or siloverallprob <= 0.0
            or siloverallprob > 1.0):
        print("{0}: error: prob is not correct in silprobs file {1}.".format(
            sys.argv[0], filename),
              file=sys.stderr)
        sys.exit(1)
    return (silbeginprob, silendcorrection, nonsilendcorrection,
            siloverallprob)
   ...
   'pron_prob', a float, is the pronunciation probability (which must be >0.0
    ...
    with open(filename, 'r', encoding='latin-1') as f:
        for line in f:
            a = whitespace.split(line.strip(" \t\r\n"))
            if len(a) < 2:
                print("{0}: error: found bad line '{1}' in lexicon file {1} ".
                      format(sys.argv[0], line.strip(" \t\r\n"), filename),
                      file=sys.stderr)
                sys.exit(1)
            word = a[0]
            ...
            if pron_prob <= 0.0:
                print(
                    "{0}: error: invalid pron-prob in line '{1}' of lexicon file {2} "
                    .format(sys.argv[0], line.strip(" \t\r\n"), filename),
                    file=sys.stderr)
                sys.exit(1)
            if len(prons) == 0:
                found_empty_prons = True
            ans.append((word, pron_prob, word_sil_prob, sil_word_correction,
                        non_sil_word_correction, prons))
            if pron_prob > 1.0:
...
def write_nonterminal_arcs(start_state, sil_state, non_sil_state, next_state,
                           sil_phone, nonterminals, left_context_phones):
    ...
    for left_context_phone in left_context_phones:
        ...
        dest = (sil_state
                if left_context_phone == sil_phone else non_sil_state)
...
def write_fst(lexicon,
    ...
    for (word, pronprob, wordsilprob, silwordcorrection, nonsilwordcorrection,
         pron) in lexicon:
        ...
        for i in range(1, len(pron)):
            ...
            next_state += 1
            print("{src}\t{dest}\t{phone}\t<eps>".format(src=cur_state,
                                                         dest=new_state,
                                                         phone=pron[i]))
            cur_state = new_state
    ...
    if nonterminals is not None:
        next_state = write_nonterminal_arcs(start_state, sil_state,
                                            non_sil_state, next_state,
                                            sil_phone, nonterminals,
                                            left_context_phones)
...
def read_nonterminals(filename):
    ...
    for nonterm in ans:
        if nonterm[:9] != '#nonterm:':
            raise RuntimeError(
                "In file '{0}', expected nonterminal symbols to start with '#nonterm:', found '{1}'"
                .format(filename, nonterm))
    if len(set(ans)) != len(ans):
        raise RuntimeError(
            "Duplicate nonterminal symbols are present in file {0}".format(
                filename))
    return ans
...
def read_left_context_phones(filename):
    ...
    for s in ans:
        if len(s.split()) != 1:
            raise RuntimeError(
                "The file {0} contains an invalid line '{1}'".format(
                    filename, s))
