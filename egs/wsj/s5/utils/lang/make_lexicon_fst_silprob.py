...
def read_silprobs(filename):
    ...
    with open(filename, 'r', encoding='latin-1') as f:
        ...
        for line in f:
            ...
            if len(a) != 2:
                print(
                    "{0}: error: found bad line '{1}' in silprobs file {1} ".
   ...
   'pron_prob', a float, is the pronunciation probability (which must be >0.0
    ...
    with open(filename, 'r', encoding='latin-1') as f:
        for line in f:
            ...
            if len(a) < 2:
                print(
                    "{0}: error: found bad line '{1}' in lexicon file {1} ".
                    format(sys.argv[0], line.strip(" \t\r\n"), filename),
                    file=sys.stderr)
                sys.exit(1)
            word = a[0]
...
def write_fst(lexicon,
    ...
    if nonterminals is not None:
        next_state = write_nonterminal_arcs(
            start_state, sil_state, non_sil_state, next_state, sil_phone,
            nonterminals, left_context_phones)

    print('{src}\t{cost}'.format(src=sil_state, cost=sil_end_correction_cost))
    print('{src}\t{cost}'.format(
        src=non_sil_state, cost=non_sil_end_correction_cost))
