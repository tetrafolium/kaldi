   ...
   'pron_prob', a float, is the pronunciation probability (which must be >0.0
    ...
    with open(filename, 'r', encoding='latin-1') as f:
        ...
        for line in f:
            ...
            if len(a) < 2:
                print("{0}: error: found bad line '{1}' in lexicon file {2} ".
            ...
            try:
                ...
                print(
                    ...
                    "should be pron-prob".format(sys.argv[0],
                                                 line.strip(" \t\r\n"),
                                                 filename),
                    file=sys.stderr)
                sys.exit(1)
...
def write_nonterminal_arcs(start_state, loop_state, next_state, nonterminals,
                           left_context_phones):
    ...
    print("{src}\t{dest}\t{phone}\t{word}\t{cost}".format(src=loop_state,
                                                          dest=final_state,
                                                          phone='#nonterm_end',
                                                          word='#nonterm_end',
                                                          cost=0.0))
...
def write_fst_no_silence(lexicon, nonterminals=None, left_context_phones=None):
    ...
    if nonterminals is not None:
        next_state = write_nonterminal_arcs(loop_state, loop_state, next_state,
                                            nonterminals, left_context_phones)

    print("{state}\t{final_cost}".format(state=loop_state, final_cost=0.0))


def write_fst_with_silence(lexicon,
                           sil_prob,
                           sil_phone,
                           sil_disambig,
                           nonterminals=None,
    ...
    if sil_disambig is None:
        print('{src}\t{dest}\t{phone}\t{word}\t{cost}'.format(src=sil_state,
                                                              dest=loop_state,
                                                              phone=sil_phone,
                                                              word='<eps>',
                                                              cost=0.0))
    ...
    if nonterminals is not None:
        next_state = write_nonterminal_arcs(start_state, loop_state,
                                            next_state, nonterminals,
                                            left_context_phones)

    print("{state}\t{final_cost}".format(state=loop_state, final_cost=0.0))


def write_words_txt(orig_lines, highest_numbered_symbol, nonterminals,
                    filename):
    ...
    if args.sil_prob == 0.0:
        ...
        if not is_token(args.sil_phone):
            print("{0}: you specified --sil-prob={1} but --sil-phone is set "
                  "to '{2}'".format(sys.argv[0], args.sil_prob,
                                    args.sil_phone),
                  file=sys.stderr)
            sys.exit(1)
        if args.sil_disambig is not None and not is_token(args.sil_disambig):
            print("{0}: invalid value --sil-disambig='{1}' was specified."
                  ...
                  file=sys.stderr)
            sys.exit(1)
        write_fst_with_silence(lexicon,
                               args.sil_prob,
                               args.sil_phone,
                               args.sil_disambig,
                               nonterminals=nonterminals,
