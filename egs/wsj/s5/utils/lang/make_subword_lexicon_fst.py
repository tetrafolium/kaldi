...
def get_args():
    ...
    parser.add_argument('--sil-disambig',
                        type=str,
    ...
    parser.add_argument('--position-dependent',
                        action="store_true",
                        help="""Whether 
...
def get_suffix(phone):
    ...
    if len(phone) < 3:
        print(
            "{}: invalid phone {} (please check if the phone is position-dependent)"
            .format(sys.argv[0], phone),
            file=sys.stderr)
        sys.exit(1)
    return phone[-2:]
...
def write_fst_no_silence(lexicon, position_dependent, separator):
    ...
    for (word, pron_prob, phones) in lexicon:
        ...
        if position_dependent:
            ...
            current_state = word_start_state
            end_state = loop_state if is_end(word,
                                             separator) else word_start_state
...
def main():
    ...
    if args.sil_prob == 0.0:
        ...
        write_fst_with_silence(lexicon, args.sil_phone, args.sil_prob,
                               args.sil_disambig, args.position_dependent,
                               args.separator)
