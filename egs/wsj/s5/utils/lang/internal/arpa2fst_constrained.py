...
class ArpaModel(object):
    ...
    def Read(self, arpa_in):
        ...
        while True:
            ...
            if line == '':
                sys.exit(
                    "{0}: reading {1}, got EOF looking for \\data\\ marker.".
                    format(sys.argv[0], arpa_in))
            if line[0:6] == '\\data\\':
                break
        while True:
            ...
            if line[0:5] != 'ngram':
                sys.exit(
                    "{0}: reading {1}, read something unexpected in header: {2}"
                    .format(sys.argv[0], arpa_in, line[:-1]))
            rest = line[5:]
        ...
        while True:
            ...
            if line == '':
                sys.exit(
                    "{0}: reading {1}, found EOF while looking for \\end\\ marker."
                    .format(sys.argv[0], arpa_in))
            elif line[0:5] == '\\end\\':
                if len(self.orders) == 0:
                    sys.exit("{0}: reading {1}, read no n-grams.".format(
                        sys.argv[0], arpa_in))
                break
            else:
                cur_order += 1
                ...
                if args.verbose >= 2:
                    print(
                        "{0}: reading {1}-grams".format(
                            sys.argv[0], cur_order),
                        file=sys.stderr)
                ...
                while True:
                    ...
                    if l != cur_order + 1 and l != cur_order + 2:
                        sys.exit(
                            "{0}: reading {1}: in {2}-grams section, got bad line: {3}"
                            .format(sys.argv[0], arpa_in, cur_order,
                                    line[:-1]))
                    try:
                        prob = math.exp(float(a[0]) * log10)
                        hist = tuple(a[1:cur_order])  # tuple of strings
                        word = a[cur_order]  # a string
                        backoff_prob = math.exp(
                            float(a[cur_order + 1]) *
                            log10) if l == cur_order + 2 else None
                    ...
                    if backoff_prob != None:
                        self.orders[cur_order][
                            hist + (word, )].backoff_prob = backoff_prob
        ...
        if len(hist) == 0:
            ...
            if hist in self.orders[len(hist)]:
                ...
                if word in hist_state.word_to_prob:
                    ...
                    return hist_state.backoff_prob * self.GetProb(
                        hist[1:], word)
            else:
            ...
            if hist_len == 1:  # it's a bigram state...
                ...
                for word in word_list:
                    ...
                    if abs(cost) < 0.01 and args.verbose >= 3:
                        print(
                            "{0}: warning: very small cost {1} for {2}->{3}".
                            format(sys.argv[0], cost, context_word, word),
                            file=sys.stderr)
                    if word == '</s>':
                        ...
                        print("%d %d %s %s %.3f" % (state, next_state, word,
                                                    word, cost))
                ...
                normalization_stats[hist_len][1] += \
                    sum([self.GetProb(hist, word)
                         for word in bigram_map[most_recent_word]])
                ...
                backoff_hist = hist[1:]
                backoff_state = self.GetStateForHist(hist_to_state,
                                                     backoff_hist)
                # note: we only print the disambig symbol on the input side.
                if args.verbose >= 3 and abs(cost) < 0.001:
                    print(
                        "{0}: very low backoff cost {1} for history {2}, state = {3}"
                        .format(sys.argv[0], cost, str(hist), state),
                        file=sys.stderr)
                ...
                this_disambig_symbol = disambig_symbol if len(
                    hist_state.word_to_prob) != 0 else '<eps>'
                print("%d %d %s <eps> %.3f" % (state, backoff_state,
                                               this_disambig_symbol, cost))
        if args.verbose >= 1:
            for hist_len in range(1, len(self.orders)):
                num_states = normalization_stats[hist_len][0]
                avg_prob_sum = normalization_stats[hist_len][1] / \
                    num_states if num_states > 0 else 0.0
                print(
                    "{0}: for {1}-gram states, over {2} states the average sum of "
                    "probs was {3} (would be 1.0 if properly normalized).".
                    format(sys.argv[0], hist_len + 1, num_states,
                           avg_prob_sum),
                    file=sys.stderr)
            if num_ngrams_disallowed != 0:
                print(
                    ...
                    "normally expect all or almost all of them to be allowed)."
                    .format(num_ngrams_allowed, num_ngrams_disallowed),
                    file=sys.stderr)
