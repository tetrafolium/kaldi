...
parser.add_option(
    ...
    help=
    'Select type of activation function : (<Sigmoid>|<Tanh>|<ParametricRelu>) [default: %default]',
    default='<Sigmoid>',
    type='string')
parser.add_option(
    '--activation-opts',
    ...
    help=
    'Additional options for protoype of activation function [default: %default]',
    default='',
    type='string')
...
parser.add_option(
    ...
    dest='hid_bias_range',
    help=
    'Set bias range for hidden activations (+/- 1/2 range around mean) [default: %default]',
    default=4.0,
    type='float')
parser.add_option(
    '--param-stddev-factor',
...
parser.add_option(
    ...
    dest='bottleneck_dim',
    help=
    'Make bottleneck network with desired bn-dim (0 = no bottleneck) [default: %default]',
    default=0,
    type='int')
parser.add_option(
    '--with-dropout',
...
(feat_dim, num_leaves, num_hid_layers,
 num_hid_neurons) = [int(i) for i in args]
# End parse options
...
if o.block_softmax_dims:
    # posible separators : ',' ':'
    assert (sum(map(int, re.split("[,:]",
                                  o.block_softmax_dims))) == num_leaves)
...
if num_hid_layers == 0 and o.bottleneck_dim != 0:
    ...
    if o.bottleneck_trick:
        ...
        print(
            ...
            % (feat_dim, o.bottleneck_dim, (o.param_stddev_factor * Glorot(
                feat_dim, o.bottleneck_dim) * 0.75), 0.1))
        ...
        print(
            ...
            % (o.bottleneck_dim, num_hid_neurons, o.hid_bias_mean,
               o.hid_bias_range, (o.param_stddev_factor * Glorot(
                   o.bottleneck_dim, num_hid_neurons) * 0.75), 0.1, 0.1,
               o.max_norm))
    else:
        ...
        print(
            ...
            % (o.bottleneck_dim, num_hid_neurons, o.hid_bias_mean,
               o.hid_bias_range, (o.param_stddev_factor * Glorot(
                   o.bottleneck_dim, num_hid_neurons)), o.max_norm))
    print("%s <InputDim> %d <OutputDim> %d %s" %
          (o.activation_type, num_hid_neurons, num_hid_neurons,
           o.activation_opts))  # Non-linearity
    ...
    print(
        ...
        % (num_hid_neurons, num_leaves, 0.0, 0.0,
           (o.param_stddev_factor * Glorot(num_hid_neurons, num_leaves)), 1.0,
           0.1))
    # Optionaly append softmax
    if o.with_softmax:
        if o.block_softmax_dims == "":
            print("<Softmax> <InputDim> %d <OutputDim> %d" % (num_leaves,
                                                              num_leaves))
        else:
...
if o.with_dropout:
    print("<Dropout> <InputDim> %d <OutputDim> %d %s" %
    ...
    if o.bottleneck_trick:
        ...
        print(
            ...
            % (num_hid_neurons, o.bottleneck_dim,
               (o.param_stddev_factor * Glorot(num_hid_neurons,
                                               o.bottleneck_dim) * 0.75), 0.1))
        # 25% smaller stddev -> smaller gradient in prev. layer, 10x smaller learning rate for weigts & biases
        print(
