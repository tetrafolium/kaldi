...
def fill_ctm(input_ctm_file, output_ctm_file, recording_names):
    recording_index = 0
    with open(input_ctm_file, "r") as infile, open(output_ctm_file,
                                                   "w") as outfile:
        for line in infile:
...
if __name__ == "__main__":
  ...
  that the ctm file has recordings in the same order as the wav.scp file"""
    ...
    parser.add_argument('recording_name_file',
                        help='file with names of the recordings')

    params = parser.parse_args()

    try:
        file_names = [
            x.strip() for x in open("{0}".format(
                params.recording_name_file)).readlines()
        ]
    except IOError:
        raise Exception("Expected to find {0}".format(
            params.recording_name_file))
