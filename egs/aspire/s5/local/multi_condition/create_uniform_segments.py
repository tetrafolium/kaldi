...
def segment(total_length, window_length, overlap=0):
    ...
    segments = [(x * increment, min(total_length, (x * increment) + window_length))
...
def prepare_segments_file(kaldi_data_dir, window_length, overlap):
    ...
    for i in range(0, len(ids)):
        ...
        for segment in segments:
            segment_string = "{0}-{1:06}-{2:06} {0} {3} {4}".format(ids[i], int(
                segment[0] * 1000), int(segment[1] * 1000), segment[0], segment[1])
