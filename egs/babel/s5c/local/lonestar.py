...
def tail(n, filename):
    import subprocess
    p = subprocess.Popen(['tail', '-n', str(n), filename],
...
def KaldiLauncher(lo, **kwargs):
    ...
    ce = SSHExecutor(
        workdir=qdir, debug=debug, force_workdir=True, catch_output=True)
    ce.outstring = "out."
    ...
    job = LauncherJob(
        hostpool=hostpool, taskgenerator=tasks, debug=debug, **kwargs)
...
def CmdLineParser(argv):
    ...
    if jobstart > jobend:
        sys.stderr.write("lonestar.py: JOBSTART(" + str(jobstart) +
                         ") must be lower than JOBEND(" + str(jobend) + ")\n")
        sys.exit(1)
...
def create_scriptfile(scriptname, opts):
    ...
    f.write("echo '#' Accounting time=$(($time2 - $time1)) threads=" +
            str(nof_threads) + " >> " + logfile + "\n")
