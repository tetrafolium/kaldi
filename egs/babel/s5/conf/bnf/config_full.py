...
class BnfExpConfig(object):
    def __init__(self):
        ...
        # finetuning data
        self.finetune_train_data = self.wdir + \
            'train.pfile.gz'   # finetune training data
        self.finetune_valid_data = self.wdir + \
            'valid.pfile.gz'   # finetune validation data
        ...
        # number of output targets.. we'll replace this with
        self.n_outs = N_OUTS
        # the correct number when we move this to the right place.
        self.hidden_layers_sizes = [1024, 1024, 1024, 1024, 1024, 42,
                                    1024]  # hidden layer sizes
        # the index of the Bottleneck layer
        self.bnf_layer_index = 6
        # number of hidden layers to be pretrained
        self.pretrain_layer_num = 5
        ...
        # learning rate for Gaussian-Bernoulli RBM
        self.pretrain_gbrbm_lr = 0.005
        # learning rate for Bernoulli-Bernoulli RBM
        self.pretrain_rbm_lr = 0.08
        self.initial_momentum = 0.5  # initial momentum
        self.final_momentum = 0.9  # final momentum
        # for how many epochs do we use initial_momentum
        self.initial_momentum_epoch = 2
        ...
        self.lrate = LearningRateExpDecay(
            ...
            min_derror_ramp_start=
            0.01,  # min validation error difference to trigger ramping
            # min validation error difference to stop finetuning, after ramping
            min_derror_stop=0.01,
            init_error=100)
