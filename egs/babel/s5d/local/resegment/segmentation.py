...
class Analysis(object):
    ...
    def write_length_stats(self, file_handle=sys.stderr):
        for i in range(0, 9):
            ...
            if use_numpy:
                try:
                    self.percentile25[i] = np.percentile(
                        self.state_count[i], 25)
                except ValueError:
                    self.percentile25[i] = 0
                try:
                    self.percentile50[i] = np.percentile(
                        self.state_count[i], 50)
                except ValueError:
                    self.percentile50[i] = 0
                try:
                    self.percentile75[i] = np.percentile(
                        self.state_count[i], 75)
                except ValueError:
            ...
            file_handle.write(
                ...
                (self.file_id, self.prefix, i, self.min_length[i],
                 self.max_length[i], self.mean_length[i], self.percentile25[i],
                 self.percentile50[i], self.percentile75[i]))
...
class JointResegmenter(object):
    def __init__(self,
        ...
        self.hard_max_frames = int(options.hard_max_segment_length /
                                   options.frame_shift)
    ...
    def get_initial_segments(self):
        ...
        self.C = ["0"] * self.N
        C = self.C
        a = Analysis(self.file_id, self.frame_shift,
                     "Analysis after get_initial_segments")
        ...
        if self.reference != None:
            ...
            for j in range(0, 9):
                a.confusion_matrix[j] = sum(
                    [C[i] == str(j) for i in range(0, self.N)])
    ...
    def set_nonspeech_proportion(self):
        ...
        target_segment_frames = int(num_speech_frames /
                                    (1.0 - self.options.silence_proportion))
        ...
        C = self.C
        a = Analysis(self.file_id, self.frame_shift,
                     "Analysis after set_nonspeech_proportion")
    ...
    def merge_segments(self):
        ...
        if self.min_inter_utt_nonspeech_length > 0.0:
            segment_starts = list(
                set([0] + segment_starts + segment_ends + [self.N]))
            segment_starts.sort()
            segment_starts.pop()
            segment_ends = list(
                set([0] + segment_starts + segment_ends + [self.N]))
    ...
    def remove_noise_only_segments(self):
        ...
        if self.reference != None and self.options.verbose > 3:
            a = self.segmentation_analysis(
                "Analysis after remove_noise_only_segments")
            a.write_confusion_matrix()
    ...
    def transition_type(self, j):
        ...
        if self.A[j - 1] in (self.THIS_SPEECH_THAT_NOISE +
                             self.THIS_SPEECH_THAT_SIL) and self.A[j] in (
                                 self.THIS_SPEECH_THAT_NOISE +
                                 self.THIS_SPEECH_THAT_SIL):
            return 0
        ...
        if self.A[j - 1] in (self.THIS_SPEECH +
                             self.THIS_NOISE_CONVERT) and self.A[j] in (
                                 self.THIS_SPEECH + self.THIS_NOISE_CONVERT):
            return 3
        if self.A[j -
                  1] in (self.THIS_SPEECH + self.THIS_NOISE_CONVERT +
            ...
            return 4
        if self.A[j - 1] in (self.THIS_SPEECH +
                             self.THIS_CONVERT) and self.A[j] in (
    ...
    def print_segments(self, out_file_handle=sys.stdout):
        ...
        max_end_time_hundredths_second = int(100.0 * self.frame_shift *
                                             max_end_time)
        num_digits = 1
        i = 1
    ...
    def segmentation_analysis(self, title="Analysis"):
        ...
        for i, st in enumerate(segment_starts):
            ...
            D[st] = (en, types.get("0", 0), types.get("1",
                                                      0), types.get("2", 0))
...
def main():
    ...
    parser.add_argument(
        ...
        default=None,
        help=
        "Maximum length in seconds of a particular speech phone prediction." +
    ...
    parser.add_argument('output_segments',
                        nargs='?',
                        default="-",
    ...
    for f in pred_files:
        ...
        if options.isolated_resegmentation or f2 not in pred_files or f1 not in pred_files:
            pred_files[f] = True
            try:
                A = open(os.path.join(prediction_dir, f +
                                      ".pred")).readline().strip().split()[1:]
            except IndexError:
            ...
            pred_files[f2] = True
            try:
                A1 = open(os.path.join(prediction_dir, f1 +
                                       ".pred")).readline().strip().split()[1:]
            except IndexError:
                sys.stderr.write("Incorrect format of file %s/%s.pred\n" %
                                 (prediction_dir, f1))
                sys.exit(1)
            try:
                A2 = open(os.path.join(prediction_dir, f2 +
                                       ".pred")).readline().strip().split()[1:]
            except IndexError:
                sys.stderr.write("Incorrect format of file %s/%s.pred\n" %
                                 (prediction_dir, f2))
            ...
            if (len(A1) -
                    len(A2)) > options.max_length_diff / options.frame_shift:
                sys.stderr.write(
