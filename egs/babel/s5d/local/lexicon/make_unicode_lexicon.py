...
def parse_input():
    ...
    if len(sys.argv[1:]) == 0:
        print(
            "Usage: ./make_unicode_lexicon.py [opts] lex_in lex_out",
            file=sys.stderr)
        sys.exit(1)
...
def encode(unicode_transcription, tag_percentage, log=False):
    ...
    graph_counts = np.histogram(
        graph_list_int, bins=bin_edges)[0] / float(len(graph_list_int))
    ...
    if log:
        graph_counts_sorted = sorted(
            graph_counts, reverse=True, key=graph_counts.get)
        if not os.path.exists(log):
            os.makedirs(log)
        with codecs.open(
                os.path.join(log, "grapheme_histogram.txt"), "w",
                "utf-8") as fp:
