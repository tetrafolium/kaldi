...
def bmp_encoder(data, width, height):
    ...
    # 4 bytes, fullsize of file, fill in later
    bmp_header[2] = struct.pack('<I', 0)
    # 4 bytes, This data is reserved but can just be set to 0
    bmp_header[3] = struct.pack('<I', 0)
    # 4 bytes, Pixel offset, 54 bytes here
    bmp_header[4] = struct.pack('<I', header_size)
    ...
    for i in range(height):
        ...
        while len(bmp_body) % 4 != 0:
            ...
            bmp_body.append(struct.pack('<B', 0))
