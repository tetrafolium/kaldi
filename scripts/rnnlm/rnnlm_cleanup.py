...
class IterationInfo:
    ...
    def __str__(self):
        return "{model_files: %s, compute_prob: %s, objf: %2.3f}" % (
            self.model_files, self.compute_prob_done, self.objf)
...
def get_compute_prob_info(log_file):
    ...
    try:
        ...
        print(
            script_name +
            ": warning: compute_prob log not found for iteration " + str(iter)
            + ". Skipping",
            file=sys.stderr)
        ...
        if objf_m is not None:
            try:
                ...
                sys.exit(
                    ...
                    .format(log_file, line, str(e)))
        if "# Ended" in line:
            compute_prob_done = True
    if objf == -2000:
        print(
            script_name + ": warning: could not parse objective function from "
            + log_file,
            file=sys.stderr)
    return iteration, objf, compute_prob_done
...
def get_iteration_files(exp_dir):
    ...
    for log in compute_prob_logs:
        ...
        if compute_prob_done:
            ...
            model_files.extend(
                glob.glob("{0}/word_embedding.{1}.mat".format(
                    exp_dir, iteration)))
            model_files.extend(
                glob.glob("{0}/word_embedding.{1}.[0-9]*.mat".format(
                    exp_dir, iteration)))
            model_files.extend(
                glob.glob("{0}/feat_embedding.{1}.mat".format(
                    exp_dir, iteration)))
            model_files.extend(
                glob.glob("{0}/feat_embedding.{1}.[0-9]*.mat".format(
                    exp_dir, iteration)))
            model_files.extend(
            ...
            if len(model_files) > 0:
                iterations[iteration] = IterationInfo(model_files, objf,
                                                      compute_prob_done)
    return iterations
...
def keep_best(iteration_dict):
    ...
    for iter, iter_info in iteration_dict.items():
        ...
        if objf == -2000:
            print(
                script_name + ": warning: objf unavailable for iter " +
                str(iter),
                file=sys.stderr)
            continue
